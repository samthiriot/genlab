...
/**
 * This is a <b>COPY</b> of the prefuse LabelRenderer, with an optimization 
 * of string using glyphvectors instead of strings. Does not appears to 
 * perform best than the original version today.
 * 
 * 
 ...
 */
public final class MyLabelRenderer extends AbstractShapeRenderer {

	/**
	 * Cache of strings to display as GlyphVectors
	 */
	private Map<String, GlyphVector> string2glyph = new HashMap<String, GlyphVector>(1000);
	
	private boolean useGlyphs = true;
    ...
    protected Logger logger = Logger.getLogger("gui.prefuse.labelRenderer");
    
    public static boolean hideLabelsWhenZoomOut = true;
    
    public static double hideLabelsZoomOutThreshold = 0.5;
    ...
    public void setUseGlyph(boolean newUseGlyphs) {
    	
    	if (this.useGlyphs == newUseGlyphs)
    		return;	// quick exit if nothing to do
    	
    	logger.debug("parameter changed: display text as glyph vectors is now "+newUseGlyphs);
    	this.useGlyphs = newUseGlyphs;
    	if (!useGlyphs) {
    		clearGlyphs();
    	}
    ...
    public void render(Graphics2D g, VisualItem item) {
        ...
        if ( type==RENDER_TYPE_FILL || type==RENDER_TYPE_DRAW_AND_FILL ) {
        	Object savedAntialiasing = g.getRenderingHint(RenderingHints.KEY_ANTIALIASING);
        	if (savedAntialiasing != RenderingHints.VALUE_ANTIALIAS_OFF)
                g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
            GraphicsLib.paint(g, item, shape, getStroke(item), RENDER_TYPE_FILL);
            if (savedAntialiasing != RenderingHints.VALUE_ANTIALIAS_OFF)
            	g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, savedAntialiasing);
        ...
        // ... or not is useless (will save cpu time !)
        if ( !(hideLabelsWhenZoomOut && g.getTransform().getScaleX() < hideLabelsZoomOutThreshold) ){
            ...
            boolean useInt, double x, double y, double w)
    {
        ...
        //g.drawGlyphVector(glyph, (float)tx, (float)y);
        
        // use integer precision unless zoomed-in
        // results in more stable drawing
        if ( useInt ) {
        	//g.drawGlyphVector(glyph, x, y);
        //	logger.debug("displaying text "+text+" as int");
        	
        	if (useGlyphs) {
        		final GlyphVector glyph = getVectorForString(text, g);
            	g.drawGlyphVector(glyph, (float)tx, (float)y);	
        	} else {
        	...
        	if (useGlyphs) {
        	//	logger.debug("displaying text "+text+" as glyphs");
        		final GlyphVector glyph = getVectorForString(text, g);
            	g.drawGlyphVector(glyph, (float)tx, (float)y);	
        	} else {
        	//	logger.debug("displaying text "+text+" as string");
    ...
    private void clearGlyphs() {
    	logger.debug("clearing glyphs ("+string2glyph.size()+" cached)");
    	string2glyph.clear();
    }
    
    private final GlyphVector getVectorForString(String str, Graphics2D graphics) {
    	
    	GlyphVector glyph = string2glyph.get(str);
    	if (glyph == null) {
    		glyph = graphics.getFont().createGlyphVector(graphics.getFontRenderContext(), str);
			string2glyph.put(str, glyph);
			//logger.debug("created glyph for "+str);
    	}
